...
describe('Test text element', () => {
  it('should update font size via keyboard', async () => {
    ...
    Keyboard.withModifierKeys({ shift: true, ctrl: true }, () => {
      ...
      expect((window.h.elements[0] as ExcalidrawTextElement).fontSize).toBe(origFontSize * 1.1)
      Keyboard.keyDown(KEYS.CHEVRON_LEFT)
      expect((window.h.elements[0] as ExcalidrawTextElement).fontSize).toBe(origFontSize)
    })
  })
})
