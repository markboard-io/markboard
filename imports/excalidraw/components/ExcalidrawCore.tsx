...
export class ExcalidrawCore extends React.Component<AppProps, AppState> {
  ...
  public render() {
    const selectedElement = getSelectedElements(this.scene.getNonDeletedElements(), this.state)
    const { onCollabButtonClick, renderTopRightUI, renderCustomStats } = this.props
  ...
  private addEventListeners() {
    ...
    this.excalidrawContainerRef.current?.addEventListener(EVENT.WHEEL, this.onWheel, {
      passive: false
    })
  ...
  public pasteFromClipboard = withBatchedUpdates(async (event: ClipboardEvent | null) => {
      ...
      selectGroupsForSelectedElements(
        {
          ...
          openSidebar:
            this.state.openSidebar && this.device.canDeviceFitSidebar && this.state.isSidebarDocked
  ...
  private addTextFromPaste(text: string, isPlainPaste = false) {
    ...
    this.setState({
      selectedElementIds: Object.fromEntries(textElements.map(el => [el.id, true]))
    })
   ...
   */
  public toggleMenu = (type: 'library' | 'customSidebar', force?: boolean): boolean => {
    if (type === 'customSidebar' && !this.props.renderSidebar) {
      console.warn('attempting to toggle "customSidebar", but no "props.renderSidebar" is defined')
      return false
    }
  ...
  private onKeyDown = withBatchedUpdates((event: React.KeyboardEvent | KeyboardEvent) => {
    ...
    if (isArrowKey(event.key)) {
      ...
      const selectedElements = getSelectedElements(this.scene.getNonDeletedElements(), this.state)
  ...
  private handleEraser = (
    ...
    const elements = this.scene.getElementsIncludingDeleted().map(ele => {
      const id =
        isBoundToContainer(ele) && idsToUpdate.includes(ele.containerId) ? ele.containerId : ele.id
  ...
  private maybeCleanupAfterMissingPointerUp(event: React.PointerEvent<HTMLCanvasElement>): void {
  ...
  private updateGestureOnPointerDown(event: React.PointerEvent<HTMLCanvasElement>): void {
    gesture.pointers.set(event.pointerId, {
      x: event.clientX,
      y: event.clientY
    })
  ...
  private initialPointerDownState(event: React.PointerEvent<HTMLCanvasElement>): PointerDownState {
      ...
      if (
        ...
        if (selectedElements.length > 0 && !pointerDownState.withCmdOrCtrl) {
          ...
          if (event.altKey && !pointerDownState.hit.hasBeenDuplicated) {
            ...
            const hitElement = pointerDownState.hit.element
            const elements = this.scene.getElementsIncludingDeleted()
            const selectedElementIds: Array<ExcalidrawElement['id']> = getSelectedElements(
              elements,
              this.state,
              true
            ...
            selectGroupsForSelectedElements(
              {
                ...
                showHyperlinkPopup:
                  elementsWithinSelection.length === 1 && elementsWithinSelection[0].link
                    ? 'info'
                    : false,
  ...
  private onPointerUpFromPointerDownHandler(
    ...
    return withBatchedUpdates((childEvent: PointerEvent) => {
          ...
          if (this.state.selectedElementIds[hitElement.id]) {
            if (isSelectedViaGroup(this.state, hitElement)) {
              ...
              const idsOfSelectedElementsThatAreInGroups = hitElement.groupIds
                .flatMap(groupId => getElementsInGroup(this.scene.getNonDeletedElements(), groupId))
      ...
      if (!activeTool.locked && activeTool.type !== 'freedraw' && draggingElement) {
        this.setState(prevState => ({
          selectedElementIds: {
            ...prevState.selectedElementIds,
            [draggingElement.id]: true
  ...
  private insertImageElement = async (
    ...
    this.scene.replaceAllElements([...this.scene.getElementsIncludingDeleted(), imageElement])
  ...
  private initializeImageDimensions = (
    ...
    const image =
      isInitializedImageElement(imageElement) && this.imageCache.get(imageElement.fileId)?.image
  ...
  private updateBindingEnabledOnPointerMove = (event: React.PointerEvent<HTMLCanvasElement>) => {
