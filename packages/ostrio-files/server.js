...
const createIndex = async (_collection, keys, opts) => {
  ...
  try {
    ...
    if (e.code === 85) {
      ...
      Meteor._debug(
        `Can not set ${Object.keys(keys).join(' + ')} index on "${_collection._name}" collection`,
        { keys, opts, details: e }
      )
    }
  }
...
class FilesCollection extends FilesCollectionCore {
  constructor(config) {
    ...
    this._checkAccess = http => {
      if (this.protected) {
        ...
        if (helpers.isFunction(this.protected)) {
          ...
          result = http
            ? this.protected.call(Object.assign(http, { user, userId }), fileRef || null)
    ...
    WebApp.connectHandlers.use((httpReq, httpResp, next) => {
      ...
      if (
        !this.disableUpload &&
        httpReq._parsedUrl.path.includes(`${this.downloadRoute}/${this.collectionName}/__upload`)
      ) {
      ...
      _methods[this._methodNames._Abort] = function (_id) {
        ...
        self._debug(
          `[FilesCollection] [Abort Method]: ${_id} - ${
            helpers.isObject(_continueUpload.file) ? _continueUpload.file.path : ''
          }`
        )
  ...
  _prepareUpload(opts = {}, userId, transport) {
    ...
    if (this.onBeforeUpload && helpers.isFunction(this.onBeforeUpload)) {
      ...
      if (isUploadAllowed !== true) {
        throw new Meteor.Error(
          403,
          helpers.isString(isUploadAllowed) ? isUploadAllowed : '@onBeforeUpload() returned false'
        )
      } else {
        if (
          opts.___s === true &&
            ...
            if (response && response.status === 200) {
              ...
              fs.remove(opts.path, removeError => {
                bound(() => {
                  ...
                  if (removeError) {
                    this._debug(
                      `[FilesCollection] [load] [fetch(${url})] [fs.remove(${opts.path})] removeError:`,
                      removeError
                    )
                  }
                })
    ...
    fs.stat(path, (statErr, stats) =>
      bound(() => {
        if (statErr) {
          ...
          callback &&
            callback(
              new Meteor.Error(400, `[FilesCollection] [addFile(${path})]: File does not exist`)
            )
        }
      })
    )
  ...
  download(http, version = 'original', fileRef) {
    ...
    if (!vRef || !helpers.isObject(vRef)) {
      ...
      if (
        ...
        !this.downloadCallback.call(Object.assign(http, this._getUser(http)), fileRef)
      ) {
        return this._404(http)
      }
  ...
  serve(
    ...
    const dispositionName = `filename=\"${encodeURI(vRef.name || fileRef.name).replace(
      /\,/g,
      '%2C'
    )}\"; filename*=UTF-8''${encodeURIComponent(vRef.name || fileRef.name)}; `
    ...
    switch (responseType) {
      ...
      case '206':
        ...
        respond(
          readableStream ||
            fs.createReadStream(vRef.path, { start: reqRange.start, end: reqRange.end }),
          206
        )
        break
      default:
