          ...
          if (statError || !stats.isFile()) {
            ...
            try {
              ...
              throw new Meteor.Error(
                500,
                `[FilesCollection] [writeStream] [constructor] [mkdirSync] ERROR: can not make/ensure directory "${paths.join(
                  nodePath.sep
                )}"`,
                mkdirError
              )
            ...
            try {
              ...
              throw new Meteor.Error(
                500,
                `[FilesCollection] [writeStream] [constructor] [writeFileSync] ERROR: can not write file "${this.path}"`,
                writeFileError
              )
            }
          }
          ...
          fs.open(this.path, 'r+', this.permissions, (oError, fd) => {
            bound(() => {
              if (oError) {
                ...
                throw new Meteor.Error(
                  500,
                  '[FilesCollection] [writeStream] [constructor] [open] [Error:]',
                  oError
                )
              } else {
                this.fd = fd
