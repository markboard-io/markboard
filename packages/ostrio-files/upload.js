...
const _rootUrl = (
  window.__meteor_runtime_config__.MOBILE_ROOT_URL || window.__meteor_runtime_config__.ROOT_URL
).replace(/\/+$/, '')
    ...
    if (this.config.file) {
      ...
      if (this.collection._supportWebWorker && this.config.allowWebWorkers) {
        try {
          ...
          this.worker = false
          this.collection._debug(
            "[FilesCollection] [insert] [create WebWorker]: Can't create WebWorker, fallback to MainThread",
            wwError
          )
        }
      } else {
      ...
      this.FSName = this.collection.namingFunction
        ? this.collection.namingFunction(this.fileData)
        : this.fileId
      this.pipes = []
      ...
      this.beforeunload = e => {
        const message = helpers.isFunction(this.collection.onbeforeunloadMessage)
          ? this.collection.onbeforeunloadMessage.call(this.result, this.fileData)
          : this.collection.onbeforeunloadMessage
  ...
  sendChunk(evt) {
    ...
    if (opts.binData) {
      if (this.config.transport === 'ddp') {
        ...
        fetch(
          `${_rootUrl}${this.collection.downloadRoute}/${this.collection.collectionName}/__upload`,
          {
            method: 'POST',
            ...
            headers: {
              'x-mtok':
                (helpers.isObject(Meteor.connection) ? Meteor.connection._lastSessionId : void 0) ||
                null,
          ...
          .then(response => {
            ...
            if (!this.config.isEnded) {
              if (response.status === 204) {
                this.collection._debug(
                  '[FilesCollection] [sendChunk] [fetch()] [then] chunk successfully sent'
                )
                this.transferTime += Date.now() - this.startTime[opts.chunkId]
                if (++this.sentChunks >= this.fileLength) {
                  this.emit('sendEOF')
                ...
                this.emit(
                  ...
                  new Meteor.Error(
                    response.status,
                    "Can't continue upload, session expired. Please, start upload again."
                  )
                )
              }
            }
          ...
          .catch(error => {
            ...
            if (!this.config.isEnded) {
              this.collection._debug(
                '[FilesCollection] [sendChunk] [fetch()] [error] EXCEPTION while sending chunk',
                error
              )
              this.transferTime += Date.now() - this.startTime[opts.chunkId]
              Meteor.setTimeout(() => {
      ...
      if (window.FileReader) {
        ...
        fileReader.onloadend = evt => {
          this.emit('sendChunk', {
            data: {
              bin: (
                (helpers.isObject(fileReader) ? fileReader.result : void 0) ||
                (evt.srcElement ? evt.srcElement.result : void 0) ||
                (evt.target ? evt.target.result : void 0)
              ).split(',')[1],
              chunkId
            }
          })
          ...
          if (!this.config.isEnded) {
            if (response.status === 204) {
              ...
              this.emit(
                ...
                new Meteor.Error(
                  response.status,
                  "Can't start upload, make sure you're connected to the Internet. Reload the page or try again later."
                )
              )
            }
          }
  ...
  start() {
    ...
    if (this.config.onBeforeUpload && helpers.isFunction(this.config.onBeforeUpload)) {
      isUploadAllowed = this.config.onBeforeUpload.call(
        Object.assign({}, this.result, this.collection._getUser()),
        this.fileData
      )
    ...
    if (this.collection.onBeforeUpload && helpers.isFunction(this.collection.onBeforeUpload)) {
      isUploadAllowed = this.collection.onBeforeUpload.call(
        Object.assign({}, this.result, this.collection._getUser()),
        this.fileData
      )
      if (isUploadAllowed !== true) {
        return this.end(
          new Meteor.Error(
            ...
            helpers.isString(isUploadAllowed)
              ? isUploadAllowed
              : 'collection.onBeforeUpload() returned false'
          )
        )
      }
    }
    ...
    if (this.worker) {
      ...
      this.worker.onmessage = evt => {
        if (evt.data.error) {
          this.collection._debug(
            '[FilesCollection] [insert] [worker] [onmessage] [ERROR:]',
            evt.data.error
          )
