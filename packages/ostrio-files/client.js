...
class FilesCollection extends FilesCollectionCore {
  constructor(config) {
    ...
    if (!config.disableSetTokenCookie) {
      const setTokenCookie = () => {
        if (Meteor.userId()) {
          cookie.set('x_uid', Meteor.userId(), { path: '/', sameSite: 'Lax' })
        }
  ...
  remove(selector = {}, callback) {
    ...
    if (this.allowClientCode) {
      ...
      callback &&
        callback(
          new Meteor.Error(401, '[FilesCollection] [remove] Run code from client is not allowed!')
        )
...
Meteor.startup(() => {
  const _template =
    Package && Package.templating && Package.templating.Template
