...
class CssToolsMinifier {
  ...
  async minifyFiles(files, mode, postcssConfig) {
      ...
      if (verbose && !this.haveHitAnyCache) {
        ...
        setTimeout(() => {
          ...
          if (this.totalMinifiedSize !== 0) {
            ...
            stats.push(
              `reduction ${Math.round(100 - (this.totalMinifiedSize * 100) / this.totalSize)}%`
            )
          }
    ...
    if (verbose)
      process.stdout.write(
        ` > Merging [ ${files.map(({ _source: { targetPath } }) => targetPath).join(' ')} ]`
      )
    ...
    if (mode === 'development') {
      ...
      if (verbose) {
        const minifiedSize = minifiedFiles.reduce(
          (sum, minifiedFile) => sum + minifiedFile.length,
          0
        )
  ...
  async processFilesForBundle(files, { minifyMode }) {
    ...
    stylesheets.forEach((stylesheet, i) => {
      if (verbose && !this.haveHitAnyCache)
        process.stdout.write(`Adding CSS${i === 0 ? '' : ' ' + i + 1}`)
...
const mergeCss = Profile('mergeCss', async function (css, postcssConfig) {
  ...
  const newMap = await Profile.time('composing source maps', async function () {
    ...
    await Promise.all(
      Object.entries(originals).map(async ([name, file]) => {
